...
import { getProduction } from "@App/common";
...
describe("testing getProduction function", () => {
  it("NoSpawns", () => {
    ...
    expect(getProduction(spawns)).toEqual([]);
  });
  it("NotSpawning", () => {
    let spawns = [new MockSpawner(null)];
    expect(getProduction(spawns)).toStrictEqual([]);
  });
  it("Spawning", () => {
    let creep = {
      id: "test"
    ...
    let expected = [creep];
    expect(getProduction(spawns)).toStrictEqual(expected);
  });
});
